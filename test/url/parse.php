...
$_SERVER['HTTP_HOST'] = 'current.subdomain.domain';
...
foreach ($urls as $c_value => $c_expected) {
    ...
    $c_result = $c_info->protocol === $c_expected['protocol'] &&
                $c_info->domain   === $c_expected['domain']   &&
                $c_info->path     === $c_expected['path']     &&
                $c_info->query    === $c_expected['query']    &&
                $c_info->anchor   === $c_expected['anchor'];
    if ($c_result)
